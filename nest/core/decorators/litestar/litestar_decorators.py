...
from litestar.controller import Controller as LitestarController
...
def Controller(prefix: Optional[str] = None, tag: Optional[str] = None):
    ...
    routes using Litestar decorators.
    ...
    def wrapper(cls: Type) -> type:
        router = LitestarRouter(path=route_prefix, tags=[tag] if tag else None)
...
def process_prefix(route_prefix: Optional[str], tag_name: Optional[str]) -> Union[str, None]:
...
def add_routes(cls: Type, router: LitestarRouter, route_prefix: str) -> None:
...
def configure_method_route(method_function: callable, route_prefix: str) -> None:
    ...
    # remove trailing "/" from __route_path__
    if (
        method_function.__route_path__ != "/"
...
def add_route_to_router(router: LitestarRouter, method_function: callable) -> None:
